...
public class RpgJavaPI {

    static int temp_dialog = 1;
    ...
    public static void main(String[] args) throws Exception {
        ...
        switch (MenuChoice) {
            ...
            case 3:
                ...
                break;
            case 4:
                Digita("\nEsse RPG se trata de um jogo que te trará maneiras diferentes de resolver uma equação de 2° grau, pense bem nas suas escolhas mas principalmente se divirta! :)",
                        TimeUnit.MILLISECONDS, temp_dialog);
                menu();
                break;
            default:
                Digita("\nVocê escolheu uma opção inválida, por favor aceite e vamos lá de qualquer jeito",
                        TimeUnit.MILLISECONDS, temp_dialog);
                menu();
                break;
        }
    ...
    public static void jogo() throws Exception {
        ...
        Digita("Antes de mergulharmos nas profundezas desta aventura, preciso conhecer nosso protagonista. Agora nós vamos começar os desafios, mas antes, nos fale o nome do seu personagem!",
                TimeUnit.MILLISECONDS, temp_dialog);
        System.out.println("\n");
        Digita("Digite o nome do seu personagem: ", TimeUnit.MILLISECONDS, temp_dialog);
        nome_do_personagem = sc.nextLine();
        Digita("Seja bem-vindo " + nome_do_personagem + "! Vamos começar os desafios...", TimeUnit.MILLISECONDS,
                temp_dialog);
        System.out.println("\n");
        ...
        Digita("MAGO - Muito bem!\nParece que você conseguiu completar os 2 desafios\nNos despedimos aqui "
                + nome_do_personagem
                + ", encontre o Monte das Equações para completar e complete os próximos desafios, espero que nos encontremos em breve e com um final feliz nessa sua jornada!\n",
                TimeUnit.MILLISECONDS, temp_dialog);
    ...
    static void creditos() throws Exception {
        ...
        Digita("Miguel Moura\nDanillo Leite\nRafael Martins\nRafael Cruz", TimeUnit.MILLISECONDS, temp_dialog);
    ...
    static void desafio1ex2() throws Exception {
        mostrarVidas();
        Digita("Você seguiu o caminho e encontrou um mago que o chamou para perto\nEsse mago igualmente os demais foram afetados pelo feitiço e só você "
                + nome_do_personagem + " pode salva-lo\n", TimeUnit.MILLISECONDS, temp_dialog);
    ...
    static void jornada2() throws Exception {
        Digita("Enquanto você caminha em direção a FLORESTA DE DELTA, você encontra magos andando.\n" +
                ...
                "já estamos com boa parte de nossos vilarejos iluminados por sua causa, continue assim, vença o DELTA, pois se ele sair vitorioso a escuridão voltará e nunca mais sairá... \n"
                +
                "\n",
                TimeUnit.MILLISECONDS, temp_dialog);
    ...
    static void monteDasEquacoes() throws Exception {
        Digita("Você caminhou por 1 kilometro e encontrou o Monte das equações...\nNa subida para do Monte você encontra outro Mago..."
                ...
                +
                " Você precisa concluir esse desafio para nos ajudar o mais rapido possível...", TimeUnit.MILLISECONDS,
                temp_dialog);
        DELTA4();
    ...
    static void florestaDeDelta() throws Exception {
        ...
        Digita("---------------------- Floresta de Delta ----------------------\n",
                TimeUnit.MILLISECONDS, temp_dialog);

        Digita("Chegando à Floresta de Delta você já sente a presença do Delta, como uma energia que você não consegue explicar.\n"
                +
                "A escuridão por toda a floresta é algo que você nunca viu...\n"
                +
                "Sua luz desperta a fúria de Delta, que ao perceber sua chegada, se aproxima e diz...\n"
                +
                "\n"
                +
                "Delta - Olá pequeno aprendiz de magia, estou acompanhando sua trajetória há um longo tempo...\n"
                +
                "Quem você pensa que é para estar destruindo meu feitiço aos poucos ?\n"
                +
                "Sua Jornada idiota acaba agora !!\n"
                +
                "Te lanço o último e pior desafio, se me ganhar meu feitiço será quebrado...\n"
                +
                "Mas por outro lado se perder, diga adeus ao seu reino !\n", TimeUnit.MILLISECONDS, temp_dialog);

        Digita("\nDelta - Esse desafio será composto por uma equação de segundo grau, porém com um detalhe mágico...\n"
                +
                "O exercício será sorteado pela minha magia, tente a sorte...\n", TimeUnit.MILLISECONDS,
                temp_dialog);
                sorteiaEx();
                Digita("Delta - Não acredito que você me derrotou!! Algum dia irei me vingar! ", TimeUnit.MILLISECONDS, temp_dialog);
                Digita("E assim "+nome_do_personagem+" derrotou Delta, e aprisionou sua escuridão para sempre\nAgora o Reino de luminaire está a salvo e com um novo herói, onde será lembrado para sempre por todos, O grande herói: " + nome_do_personagem + "!\n", TimeUnit.MILLISECONDS, temp_dialog);
                Digita("--------------- Fim da História ---------------", TimeUnit.MILLISECONDS, temp_dialog);
    ...
    static void DELTA4() throws Exception {
        ...
        do {
            ...
            if ((x1 == (-b + Math.sqrt(delta1)) / (2 * a) && x2 == (-b - Math.sqrt(delta1)) / (2 * a))
                    || (x1 == (-b - Math.sqrt(delta1)) / (2 * a) && x2 == (-b + Math.sqrt(delta1)) / (2 * a))) {
    ...
    static void DELTA1() throws Exception {
        ...
        do {
            ...
            if ((x11 == (-b1 + Math.sqrt(delta2)) / (2 * a1) && x21 == (-b1 - Math.sqrt(delta2)) / (2 * a1))
                    || (x11 == (-b1 - Math.sqrt(delta2)) / (2 * a1) && x21 == (-b1 + Math.sqrt(delta2)) / (2 * a1))) {
                Digita("Parabéns! Você resolveu corretamente a equação e teve seu caminho iluminado com sucesso!\n",
                        TimeUnit.MILLISECONDS, temp_dialog);
            }
    ...
    static void DELTA0() throws Exception {
        ...
        do {
            ...
            if ((x12 == (-b2 + Math.sqrt(delta3)) / (2 * a2) && x22 == (-b2 - Math.sqrt(delta3)) / (2 * a2))
                    || (x12 == (-b2 - Math.sqrt(delta3)) / (2 * a2) && x22 == (-b2 + Math.sqrt(delta3)) / (2 * a2))) {
                Digita("Parabéns! Você resolveu corretamente a equação e teve seu caminho iluminado com sucesso!\n",
                        TimeUnit.MILLISECONDS, temp_dialog);
   ...
   //exercicio de equação de segundo grau
    static void ex1FlorestaDelta() throws Exception{
        int[] vetorC = {1, -9, 20};
            int delta = 1;
            double vetorX[]=new double[2];
            double vetorR[]=new double[2];
            boolean acertou=false;            
            mostrarVidas();
            do{
                System.out.println("Equação: " + vetorC[0] + "x² - 9x + " + vetorC[2] + "= 0");
                System.out.println("Digite o x1: ");
                vetorX[0] = sc.nextDouble();
                System.out.println("Digite o x2: ");
                vetorX[1]= sc.nextDouble();
                vetorR[0] = (-vetorC[1] + Math.sqrt(delta)) / (2 * vetorC[0]);
                vetorR[1] = (-vetorC[1] - Math.sqrt(delta)) / (2 * vetorC[0]);
    
                if((vetorX[0] == vetorR[0] && vetorX[1] == vetorR[1]) || (vetorX[1] == vetorR[0] && vetorX[0] == vetorR[1])) {                
                    acertou=true;
    ...
    static void ex2FlorestaDelta() throws Exception{
        int[] vetorC = {1, -8, 16};
            int delta = 0;
            double vetorX[]=new double[2];
            double vetorR[]=new double[2];
            ...
            mostrarVidas();
            do{
                System.out.println("Equação: " + vetorC[0] + "x² - 8x + " + vetorC[2] + "= 0");
                System.out.println("Digite o x1: ");
                vetorX[0] = sc.nextDouble();
                System.out.println("Digite o x2: ");
                vetorX[1]= sc.nextDouble();
    ...
    //exercicio de equação de segundo grau
    static void ex3FlorestaDelta() throws Exception{
        int[] vetorC = {1, -7, 12};
            int delta = 1;
            double vetorX[]=new double[2];
            double vetorR[]=new double[2];
            ...
            mostrarVidas();
            do{
                System.out.println("Equação: " + vetorC[0] + "x² - 7x + " + vetorC[2] + "= 0");
                System.out.println("Digite o x1: ");
                vetorX[0] = sc.nextDouble();
                System.out.println("Digite o x2: ");
                vetorX[1]= sc.nextDouble();
    ...
    // sorteia o metodo de algum exercicio 
    static void sorteiaEx()throws Exception{
